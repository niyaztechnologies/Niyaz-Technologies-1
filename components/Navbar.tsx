...
import { COMPANY_DETAILS } from '../constants.tsx';
...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-[100] bg-[#0a192f]/95 backdrop-blur-md border-b border-white/10 shadow-2xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex-shrink-0">
            <Link to="/" className="flex items-center gap-3">
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full overflow-hidden bg-white border-2 border-blue-500 flex items-center justify-center shadow-lg">
                <img 
              ...
              <span className="text-white font-black text-lg tracking-tighter uppercase hidden xs:block">
          ...
          <div className="lg:hidden">
            <button 
              ...
              className="w-11 h-11 flex items-center justify-center rounded-xl bg-blue-600 text-white shadow-xl"
            >
      ...
      {isOpen && (
        <div className="lg:hidden bg-[#0a192f] border-t border-white/10 p-6 space-y-2 shadow-2xl">
          {['/', '/about', '/pricing', '/portfolio', '/blog', '/contact'].map((path) => (
            <Link 
              key={path}
              to={path} 
